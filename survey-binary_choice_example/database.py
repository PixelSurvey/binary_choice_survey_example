...
class Database:
    ...
    def register_screening_responses(self, respondent_id:int, responses:dict):
        ...
        query = "UPDATE Response SET sq1_age_group = ?, sq2_gender = ?, sq3_continent = ? WHERE respondent_id = ?"
    ...
    def allows_respondent_to_continue(self, responses:dict):
        ...
        query = "SELECT quota - current FROM Quotas WHERE sq1_age_group = ? AND sq2_gender = ? AND sq3_continent = ?"
    ...
    def update_quota(self, respondent_id):
        ...
        query_sq_responses = "SELECT sq1_age_group, sq2_gender, sq3_continent FROM Response WHERE respondent_id = ?"
        params = (respondent_id,)
        responses = self._read_with_params(query_sq_responses, params)

        # Update quota for those responses
        query_quotas = "UPDATE Quotas SET current = current + 1 WHERE sq1_age_group = ? AND sq2_gender = ? AND sq3_continent = ?"
        params = (responses[0])
